...
Page({
    ...
    wx.login({
      success: (res) => {
        ...
        this.log(
          "wx.login 成功，code=" +
            res.code.slice(0, 12) +
            "…（后端将调用微信 code2session 换取 openid）",
        );
    ...
    this.request({ url: "/api/user/login", method: "POST", data: { code } })
      .then((body) => {
        ...
        this.log(
          "登录成功：userId=" +
            data.id +
            "，双 Token 已保存（access 2h / refresh 7d）",
          "ok",
        );
      ...
      .catch((err) => {
        ...
        if (err.message && err.message.indexOf("微信登录失败") > -1) {
          this.log(
            "提示：请确认后端已配置微信 appid / secret，且 code 未被重复使用",
            "err",
          );
        }
      });
  ...
  onRefresh() {
    ...
    this.request({
      ...
      .then((body) => {
        ...
        this.log(
          "Token 刷新成功，旧 accessToken 已作废，新 Token 已保存",
          "ok",
        );
  ...
  onChooseAvatar(e) {
    ...
    this.setData({
      ...
      avatarUrl: "",
      avatarStatus:
        "已选择头像，请点击下方“上传头像”按钮（后端调微信 imgSecCheck 审核）",
      avatarStatusClass: "",
    });
  ...
  onUploadAvatar() {
    ...
    this.setData({
      uploading: true,
      avatarStatus:
        "上传中…后端将校验大小/真实类型，并调用微信官方 imgSecCheck 内容安全接口",
      avatarStatusClass: "",
    });
    this.log(
      "步骤1：wx.uploadFile → POST /api/user/avatar（携带 token，multipart 字段 file）",
    );
    wx.uploadFile({
      ...
      success: (res) => {
        ...
        if (body.code === 1) {
          ...
          this.finishUpload(
            "头像上传失败（code=" + body.code + "）：" + body.msg,
            "err",
          );
          if (body.code === 10008) {
            this.log(
              "10008 = 微信内容安全判定违规，文件已被拦截，未进入 OSS",
              "err",
            );
          }
          if (body.code === 10009) {
            this.log(
              "提示：微信内容安全服务不可用，常见原因是 appid/secret 未配置或 access_token 获取失败",
              "err",
            );
          }
        }
  ...
  onSaveProfile() {
    ...
    this.log(
      "步骤2：POST /api/user/profile（携带 token），body=" +
